<?php
    // [TODO]
    $cLang = array(
        "languageD" => "Espa?ol",

        // [categories]
        "recentC" => "M?s reciente en FulpTube - Mundial",
        "musicC" => "M?sica",
        "reccomendedC" => "Recomendado",
        "videoC" => "Videojuegos",
        // [video] 
        "byUsr" => "de",
        "agoLeft" => "hace unos",
        "agoRight" => "", // <agoLeft> <date> <agoRight> ex: 3 seconds ago
        "justNow" => "ahora mismo",
        "ago" => "hace",
        "country" => "Pa?s",
        "eViews" => "vistas",
        "worldwide" => "En todo el mundo",
        "gotIt" => "?Lo tengo!",
        "welcomeToFulpTubeHomepage" => "Bienvenido a la p?gina principal de FulpTube",
        // [sidebar]
        "channelsForYou" => "Canales para t?",
        "dashboardText" => "Los v?deos de tus suscripciones al canal y las recomendaciones de v?deo personalizadas aparecen a continuaci?n. FulpTube clasifica estos v?deos en funci?n de lo que creemos que querr?s ver a continuaci?n.<br><br>
        ?Quieres ver toda la actividad m?s reciente de tus suscripciones?<br>
        Consulta <a href=\"#\">Mis suscripciones</a>.",
        "subscriptions" => "Subscripciones",
        "subscriptionManage" => "Gestionar Subscripciones",
        "browseChannels" => "Navegar canales",
        "newestVideos" => "V?deos m?s recientes",
        "signInNow" => "Inicia sesi?n para ver tus canales y recomendaciones!",
        // [header]
        "uploadVid" => "Subir",
        "browse" => "Navegue por",
        "vChannel" => "Canal",
        "feed" => "Actividad reciente",
        "vUploaded" => "Subido a",
        "vCancel" => "Cancelar",
        "vDuration" => "Duraci?n",
        "annoHowtoTitle" => "C?mo se usa:",
        "videoResponseStart" => "<a href=\"/videoresponse?v=",
        "videoResponseEnd" => "\">Erstellen Sie eine Videoantwort</a> oder ",
        "vViews" => "Vistas",
        "vLikes" => "Le gusta",
        "annoCreator" => "Annotation Maker de CrazyFrog",
        "vDislikes" => "No le gusta",
        "toCreateAnnotations" => "Para empezar a crear anotaciones para su v?deo, cargue un archivo.",
        "vStatus" => "Estado",
        "toggleCommenting" => "Alternar los comentarios",
        "vComments" => "Comentarios",
        "annoHowto" => 'Establezca la posici?n, la escala, el color y la duraci?n de sus anotaciones.<br><br>

        Pulse el bot?n "A?adir a la salida" para guardar los cambios.<br><br>
        
        Si no le gustan los cambios realizados, pulse el bot?n "Deshacer".<br><br>
        
        Para empezar de nuevo, pulse el bot?n "Borrar salida".<br><br>
        
        Si est? satisfecho con sus anotaciones, pulse el bot?n "Copiar al portapapeles" para copiar el contenido terminado en su portapapeles.',
        "vURL" => "URL del v?deo",
        "actualUploadVideo" => "Subir v?deo",
        "delete" => "Borrar",
        "thumbnail" => "Miniatura",
        "upload" => "Subir a",
        "signIn" => "Iniciar sesi?n",
        // [time]
        "year" => "a?o",
        "month" => "mes",
        "week" => "semana",
        "day" => "dia",
        "hour" => "hora",
        "minute" => "minuto",
        "second" => "segundo",

        "pluralTimeRight" => "s",
        // [watch page]
        "subscribe" => "Suscribir",
        "language" => "Idioma",
        "copyright" => "Copyright",
        "about" => "Acerca de",
        "shareVid" => "Compartir",
        "addTo" => "Agregar a",
        "tags" => "Palabras clave",
        "thumbnail" => "Miniatura",
        "charremaining" => "caracteres restantes",
        "rectag" => "Etiquetas recomendadas",
        "vidStatistics" => "Estad?sticas",
        "showMore" => "Ver m?s",
        "prePost" => "<a href='/videoresponse?v=%d'>Crea una v?deorespuesta</a> o",
        "post" => "post",
        "cPost" => "Post",
        "showLess" => "Mostrar menos",
        "reply" => "Responder",
        "shareyourthoughts" => "Comparte tus pensamientos",
        "allComments" => "Todos los comentarios",
        "publishedAt2" => "Publicado el",
        "topComments" => "Comentarios m?s populares",
        "loadMore" => "Mostrar m?s",
        // [profile]
        "discussion" => "Debate",
        "recentUploads" => "Subidas recientes",
        "playlists" => "Playlists",
        "profilePic" => "Imagen de perfil",
        "uploadBanner" => "Cargar la pancarta",
        "uploadImage" => "Cargar imagen",
        "joined" => "Cuenta creada en",
        "messageProfile" => "Enviar mensaje",
        "uploadedAVideo" => "subi? un v?deo",
        "subscribers" => "suscriptores",
        "userJoined" => "Se uni? el %d",
        "featuredChannel" => "Canales destacados",
        "popularChannels" => "Canales populares en FulpTube",
        "theseRecommendations" => "Estas recomendaciones han sido generadas autom?ticamente por FulpTube.",
        // [inbox]
        "newComment" => "Nuevo comentario",
        "newSubscriber" => "Nuevo suscriptor",
        "inbox" => "Bandeja de entrada",
        "markAll" => "Marcar todos como le?do",
        "compose" => "Escribir",
        "from" => "De:",
        "sent" => "Enviado",
        "sendMessage" => "Enviar mensaje",
        "date" => "Fecha",
        "to" => "Para:",
        "subject" => "Asunto:",
        "messageInbox" => "Mensaje:",
        // [upload]
        "uTitle" => "T?tulo",
        "category" => "Categor?a",
        "privacySetting" => "Configuraci?n de privacidad",
        "description" => "Descripci?n",
        // [sign in]
        "signInBenifits" => "Inicia sesi?n ahora y usa tu cuenta de FulpTube para:",
        "s1a" => "Estar al d?a con tus canales favoritos",
        "s1b" => "Guarda v?deos para ver luego, mira recomendaciones solo para t?, o suscribete para tener actualizaciones de tus canales favoritos",
        "s2a" => "Mira en todos lados", 
        "s2b" => "Toma tus selecciones contigo a cualquier lugar que vayas -- mira en tu smartphone, tableta o Smart TV.",
        "s3a" => "Comparte con tus amigos",
        "s3b" => "Mira v?deos compartidos por tus amigos a trav?s de todas tus redes sociales -- todo en un solo lugar.", 
        // [misc]
        //"" => "",
        "username2" => "Nombre de usuario",
        "password2" => "Contrase?a",
        "email2" => "Correo electr?nico",
        "confirmpw2" => "Confirmar Contrase?a",
        "register" => "Registrar",
        "signUp" => "Iniciar Sesi?n",
        "orRegister" => "...o <a href='/newsignup'>reg?strate</a>",
        "unsubscribe" => "Desuscribir",
        "addChannels" => "A?adir canales",
        "subscribed" => "Suscrito",
        "xVideos" => "v?deos",
        "uVideos" => "V?deos",
        "transcribe" => "Transcribir",
        "statistics" => "Estad?sticas",
        "report" => "Reportar",
        "popularChannels" => "Canales Populares en Fulptube",
        "featuredChannels" => "Canales Destacados",
        "dateAddedoldNew" => "Fecha de adici?n (m?s antigua - m?s reciente)",
        "dateAddedNewOld" => "Fecha de adici?n (m?s reciente - m?s antigua)",
        "Uplaylist" => "Playlists",
        "mostPopular" => "Los m?s populares",
        "privacy" => "Privacidad",
        "playlist" => "playlist",
        "termsAndCond" => "T?rminos y condiciones",
        "sendFeedback" => "Enviar comentarios",
        "recentUploads" => "Subidas Recientes",
        "uploads" => "Sube a",
        "searchSubscriptions" => "Buscar Suscripci?nes",
        "comments" => "comentario(s)",
        "inboxComments" => "Comentarios",
        "contactNotifications" => "Avisos de contacto",
        "message" => "Mensaje",
        "time" => "Tiempo:",
        "joindate" => "Se uni? el",
        "myChannel" => "Mi Canal",
        "videoManager" => "Gestor de v?deos",
        "fulptubeSettings" => "Configuraci?n",
        "userSettings" => "Configuraci?n de usuario",
        "signOut" => "Salir",
        "foogleAccount" => "Cuenta",
        "thisisyou" => "Este eres t?.",
        "overview" => "Resumen",
        "name" => "Nombre",
        "password" => "Contrase?a",
        "changePW" => "Cambiar su Contrase?a",
        "cemail" => "Se le enviar? un email confirmando el restablecimiento de su contrase?a",
        "misc" => "Misc",
        "accCreated" => "Creado en:",
        "loginDate" => "Login Previo:",
        "customization" => "Personalizaci?n",
        "personalMessages" => "Mensajes personales",
        "sharedWithYou" => "Compartido con usted",
        "banner" => "Banner",
        "resWarn" => "Nota: La resoluci?n ?ptima para los banners es 821x176. Banners de resoluci?n diferente se estirar?n para adaptarse correctamente.",
        "yourPhoto" => "Tu foto",
        "socialLinks" => "Enlaces de redes sociales",
        "links" => "Enlaces",
        "customURL" => "URL personalizada",
        "leaveBlank" => "Deja vac?o si no quieres poner esto.",
        "featureAndDescribe" => "Presentar y Describir",
        "describeAndfeature" => "Describir y Presentar",
        "sent" => "Enviado",
        "webVTT" => "Nota: S?lo se admiten subtitulos en formato WebVTT.",
        "edit" => "Editar",
        "maxRes" => "Nota: El tama?o m?ximo es de 1080p.",
        "bio" => "Biograf?a",
        "captionsNotWorking" => "Los subt?tulos no funcionan temporalmente.",
        "anno" => "Anotaciones",
        "basicInfo" => "Informaci?n b?sica",
        "videoManager" => "Gestor de v?deo",
        "displayName" => "Nombre Visible",
        "addMoreVideos" => "A?adir m?s v?deos",
        "featuredChannels" => "Canales Destacados",
        "exampleFeaturedChannels" => "Separa usando comas, no uses espacios. Ejemplo: chief bazinga,Sniped,Joell98",
        "featuredVideo" => "V?deo Destacado",
        "warningFeaturedVideo" => "No debe tener espacios/espacio extra al final. Ejemplo: MTYwNTQ4ODc4Mja=24 es v?lido. NO INCLUYAS EL ENLACE DE FULPTUBE.ROCKS, SOLO EL ID. ",
        "toggleFlash" => "Activar Adobe Flash",
        "aboutResults" => "Aproximadamente %d resultados",
        "searchSubscriptions" => "Buscar Suscripci?nes",
        "watchLater" => "Ver Luego",
        "donotreupload" => "Por favor, no resubas v?deos que no son tuyos.<br>

        Las miniaturas se generan autom?ticamente.<br>
        Todos los v?deos se convierten a 720p@1000kbps H264/AAC mediante ffmpeg, lo que permite una gran variedad de formatos.<br>
        Los v?deos deben ser menores de 100MB. (debido a Cloudflare)<br>
        Nada de contenido con derechos de autor que nos dara un DMCA.<br>
        Los v?deos se aprueban manualmente.<br>",
        "watchHistory" => "Historial",
        "whatToWatch" => "Qu? ver",
        "mySubscriptions" => "Mis Subscripci?nes",
        "analytics" => "An?lisis",
        "dashboard" => "Panel",
        "adobeStatus" => "Adobe Flash no est? ",
        "active" => "activado",
        "guide" => 'Men?',
        "deactive" => "desactivado",

        "fulptubeTOS" => "Condiciones de servicio de FulpTube",
        "fulptubeTOSSub" => "                        <b>Reglas generales</b><br>
        No seas un idiota. Este es un sitio hecho para divertirse. <br>
        Este es un sitio orientado a recrear el YouTube de 2013. <br>
        Los reproductores + JavaScript del sitio es propiedad de YouTube. No es nuestro.<br>
        FulpTube no est? asociado de ninguna manera con Google LLC.<br>
        Todo el contenido que se encuentre violando las leyes de los EE.UU. y las regulaciones federales ser? eliminado y la cuenta asociada a ella baneada.<br>
        Todos los intentos de evasi?n de ban NO SER?N TOLERADOS y terminar?n con un ban de FulpTube.<br>
        Nada de NSFW, nada de fanatismo, nada de homofobia, y s?lo usar el sentido com?n. Gracias.<br><br>
        <b>Informes de errores + informes de v?deos</b><br>
        Si ves un v?deo que incumple las TOS, notif?calo en el discord del pie de p?gina.<br>
        ?Has encontrado un bug? rep?rtalo en el canal #bug-reports en el discord.<br><br>
        <b>Contacto</b><br>
        Correo electr?nico: spacemygithub@gmail.com<br>
        Servidor oficial de Discord: En el pie de p?gina<br>
        Discord: bhief cazinga#5644",

        "ftPrivacy" => "Pol?tica de privacidad de FulpTube",
        "ftPrivacySubtext" => "                        <b>Privacidad</b><br>
        NO almacenamos las IPs. Todo lo que se almacena es tu contrase?a, correo electr?nico y nombre de usuario.<br>
        Otras cosas que se almacenan son tus v?deos, comentarios, etcettera. Puedes solicitar recuperarlos.<br>
        No vendemos datos. <br>
        ?Tienes alguna duda sobre la privacidad? La discordia se encuentra arriba.<br>",
        
        "ftCopyright" => "Derechos de autor de FulpTube",
        "ftCopyrightSubtext" => "                        <b>Derechos de autor</b><br>
        Aseg?rate de que no est?s ganando dinero con la m?sica que suena de fondo en un v?deo.<br>
        Puedes subir canciones completas sin editar siempre que des cr?dito al editor original.<br>
        No se tolera la suplantaci?n de otras empresas con marcas o derechos de autor.<br>
        Tienes que hacer que se note f?cilmente que tu canal es una parodia si lo es.",
        "ftQA" => "Preguntas y respuestas de FulpTube",
    
        "ftQASubtext" => "                        <b>Olvid? mi contrase?a.</b><br>
        ?nete al Discord para recibir ayuda con respecto a las contrase?as.<br><br>
        <b>?Por qu??</b><br>
        Hice FulpTube como un proyecto para divertirme y nunca esper? tanta tracci?n y atenci?n para ?l. <br>Mantendr? este sitio mientras pueda.<br>Deber?as unirte al Discord si quieres hablar con la comunidad.<br><br>
        <b>?Qui?n es el due?o de este sitio?</b><br>
        <a href=\"/user?n=chief%20bazinga\">chief bazinga</a><br>",
    );
?>
